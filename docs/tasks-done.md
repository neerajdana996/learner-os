...
### T-171 · `codeEditor` in the side panel, as a review everywhere, and graded in every language
- **status:** done
- **sprint:** 6
- **depends_on:** T-088, T-089, T-170
- **files:** `backend/src/lib/popupEligible.ts`, `backend/src/lib/grade.ts`, `backend/src/generator/grade.ts`, `backend/src/llm/prompts/gradeCode/`, `backend/src/llm/models.ts`, `packages/ui/src/blocks/CodeEditor.tsx`, `packages/ui/src/QuestionCard.tsx`, `extension/src/entrypoints/sandbox/`, `extension/src/lib/evaluateCases.ts`, `extension/src/lib/runInSandbox.ts`, `extension/src/card/Card.tsx`, `extension/src/lib/api.ts`
- **description:** Founder request (2026-09-14): the extension is a side panel and must show every card, `codeEditor` and `orderLines` included. Founder decisions taken while planning it: **non-JavaScript answers are judged by the model** (not executed on the host, not restricted to JS), and **`codeEditor` becomes a review on both surfaces**, not only the extension.
- **acceptance:** the panel's `/due` serves every answer format; the web session serves `codeEditor` as a review; a `codeEditor` in the panel runs, offers the skeleton and sends `assisted`; a non-JS answer is graded by what the code would do rather than always failing; the Day-30 test still refuses `codeEditor` and `orderLines`.
- **tests:** all below, passing — lint 7/7; `@learnos/ui` 48; extension 126; backend `grade`, `popupEligible`, `testGen`, `rationItems` 80; backend DB suites `due`, `items`, `session`, `tests`, `tests.worker` 70. E2E specs were updated but **not run**.
- **notes:** (2026-09-14)
  - **Eligibility:** `PANEL_INELIGIBLE_KINDS` and `REVIEW_INELIGIBLE_KINDS` are both empty; `COLD_TEST_INELIGIBLE_KINDS` is unchanged. The SQL helpers now return `undefined` for an empty list instead of emitting `NOT IN ()`, asserted by a test. `rationItems` still allows one `codeEditor` per session for newly taught concepts.
  - **T-169's skip-past regression kept its teeth:** with no format left ineligible, its blocked card is now a *retired* item, which goes through the same `servableItem()` condition.
  - **Grading was broken wider than the audit said.** TypeScript was in `RUNS_HERE`, but the runner evaluates JavaScript, so every annotated TS answer "did not run". Only `javascript` runs client-side now; every other language posts `{"__source": ...}` and `grade.ts` hands it to `gradeCode` (luna, reasoning `low` — `none` would guess at a trace). The expected outputs stay on the server; the prompt forbids quoting them in `feedback`. A case the judge omits, or a name it invents, is a failure. A judge error propagates (500 → the extension's offline queue retries), the same no-free-pass rule as `explain`. JSON that parses to a non-object is now "did not run" rather than a TypeError.
  - **The extension could never have run a `codeEditor`,** independent of eligibility: the web runner is a `srcdoc` iframe with an inline script, and an extension page's MV3 CSP blocks inline script and eval — a `srcdoc` frame inherits it. The fix is a WXT manifest sandbox page (`sandbox.html`, verified in the built manifest's `sandbox.pages`) with an opaque origin and no extension APIs, driven by `runInSandbox` over `postMessage`. `CodeEditor` and `QuestionCard` take an injectable `runCode`; the web keeps the default.
  - **Not verified:** the sandbox runner in a real Chrome (happy-dom cannot load it — the e2e spec `a due codeEditor opens in the panel and can be submitted` covers it and has not been run), and `gradeCode` against the live model. An infinite loop in the sandbox page may freeze the panel until the 2s timeout removes the frame, the same exposure the web runner already has.
  - Not done: the web session still passes no `onSkeleton`, so the hint is offered only in the extension. Pre-existing, not widened here.
  - (2026-09-15) **Proven and hardened.**
    - **E2E run:** 56 passed, 1 skipped (`session.spec.ts:197`, a pre-existing conditional skip when no `try_first` concept is due). All four T-171 specs passed, including the codeEditor opening, running and submitting in the side panel in real Chrome.
    - **`gradeCode` proven against the live model** with `pnpm eval:grade-code` (`backend/src/scripts/evalGradeCode.ts`): 23 labelled answers in Python, TypeScript, Java, Go and C++, 2 runs each — correct code in two styles, off-by-one, hard-coded return, missing return, syntax error, infinite loop, pseudocode, three prompt-injection attempts (comment, fake closing tag, string literal), a binary-search bug that only shows on the last element, Java int overflow, a Go nil-map write, and float/list-format equivalence. **46/46 answers and 112/112 cases agreed with the labels; 0 correct answers failed, 0 wrong cases passed, 0 errors, 0 feedback lines quoting an expected output.** p50 2.0s, p90 2.7s, max 3.3s; **$0.00024 per answer**. The script exits non-zero below 95% agreement, so it gates any prompt or model change.
    - **Hardening:** strict `json_schema` output on `gradeCode` (no malformed-JSON retry doubling latency); `timeoutMs: 20_000` and `maxRetries: 1` on both grading prompts, via new `PromptDef.timeoutMs`/`maxRetries` forwarded to the SDK — the default was a ten-minute timeout with two retries on a request a learner is waiting on; source over `MAX_JUDGED_SOURCE` (6,000 chars) or blank is refused before any model call, because the answer field has no length limit.
    - **Web session fixes found while checking the failure path:** a failed Check showed nothing at all (the rejection was unhandled), and Check stayed clickable while grading — web answers carry no idempotency key, so a second click recorded a second review and scheduled the card twice. Both buttons now wait for the pending submit, and a failure shows an alert with the answer kept on screen.
...
- **sprint:** 4
- **depends_on:** T-013
- **files:** `backend/src/modules/users/*`, `packages/shared/src/schemas.ts`, `packages/shared/src/types.ts`, `frontend/src/features/users/usersApi.ts`, `frontend/src/app/AccountMenu.tsx`, `frontend/src/styles/components/_bar.scss`, `frontend/src/features/legal/pages/PrivacyPage.tsx`, `frontend/src/features/onboarding/pages/OnboardingPage.tsx`
...
- **notes:** (2026-09-14) **The public half landed; the API half has not.** `/privacy`, `/terms` and `/contact` exist as public routes outside `RequireAuth`, with a shared footer carrying the operator, location and a contact address — the site previously had no legal page, no contact route and no email address anywhere, which is a gap for a product that asks for an email address before it does anything.
  ...
  - (2026-09-15) **Done: both routes, the account-menu buttons, and the onboarding note.**
    - **`DELETE /me`** takes `{ confirmEmail }` (the learner types their own address; compared case-insensitively) and removes them from **every table in one transaction**. **No foreign key in `schema.ts` cascades**, and adding cascades would be a schema change, so `deleteUserAndEverythingTheyOwn` deletes children first in the order the seed scripts already used. It deletes every session, so the extension's next request is a 401 that clears its token, and it clears the session cookie like `/auth/logout`. A mismatched or missing confirmation deletes nothing.
    - **A guard test** reads `information_schema` and asserts the exact set of tables with a foreign key to `users`, `topics`, `concepts`, `items` or `cards`. A table added later without a line in the deletion fails that test by name, instead of making `DELETE /me` fail in production with a constraint error.
    - **`GET /me/export`** returns everything held, as an attachment, with a `counts` object (the acceptance's review_events count). It **deliberately leaves out** item payloads (answer keys to questions the Day-30 test asks), held-out concept titles (the control arm), and credential hashes; the repository reads those tables with explicit column lists so a later column cannot leak, and the service parses the output through `MeExportSchema`, so an undecided field is dropped rather than sent. Tested with marker strings in all three places.
    - **UI:** "Download my data" and "Delete my account" in the account menu, in production (unlike the dev reset tools). Deletion is locked until the typed address matches; afterwards the whole RTK Query state is reset so no screen renders the deleted account. The privacy page now describes the buttons and what the copy omits, and its extension permission list gained `sidePanel` (missing since T-170). The last onboarding step carries the plain-language note, linking `/privacy`.
    - `curl` examples:
      - `curl -s https://api.coldrecall.info/me/export -H 'Cookie: learnos_session=<token>' -o cold-recall-data.json`
      - `curl -s -X DELETE https://api.coldrecall.info/me -H 'Cookie: learnos_session=<token>' -H 'Content-Type: application/json' -d '{"confirmEmail":"you@example.com"}'`
    - **Deviations from `files`:** there is no `docs/privacy.md` — the policy is the public page `PrivacyPage.tsx` — and the routes live in `backend/src/modules/users/`, not `backend/src/routes/users.ts`.
    - **Not handled:** deleting an account while one of its topics is still generating. The worker's later inserts reference a topic that no longer exists and the job fails on the foreign key — loudly, leaving nothing behind — rather than being cancelled.
    - **Not verified in a browser:** the two menu items have no frontend unit or e2e test; their API paths are fully tested.
    - **Found while doing it:** T-174 (the onboarding screen still promises the old 29-day course and a Day-45 test) and T-FIX-013 (below).
    - **Tests:** users suite 28, twelve new; full backend **695/695**; frontend 84; extension 126; `@learnos/ui` 48; `@learnos/shared` 54; lint 7/7; the frontend build compiles the new SCSS.

### T-FIX-013 · The generator worker tests call the real model provider
- **status:** done
- **severity:** high — a network stall turns the backend suite red for fifteen minutes, and the failure names unrelated tests
- **depends_on:** T-166
- **files:** `backend/src/workers/__tests__/generator.worker.test.ts`, `backend/src/integration/sprint1.test.ts`, `backend/src/integration/sprint2.test.ts`
- **description:** Found closing T-046 (2026-09-15). A full `pnpm test` failed nine backend tests — eight in `generator.worker.test.ts` and `POST /reviews > 404s for an unknown item`, which ran for **~15 minutes** against a 20s timeout. Both files pass when run alone, and the test database held no stuck transactions.
  - **The cause:** T-166 added `enrichConceptItems` (`generator/itemBlocks.ts`) to the worker, and the worker test mocks `conceptMap`, `framing`, `items` and `teaching` but **not `itemBlocks`**, so every generation test runs the real enrichment call. *(Corrected the same day: this was first written up as a live network call that stalled under the SDK's ten-minute timeout. It was not.)* T-FIX-009's `fetch` guard in `vitest.setup.ts` refuses the request before anything leaves the machine. The OpenAI SDK reads that refusal as a connection failure and **retries it twice with exponential backoff** (0.5s initial, 8s cap), and `enrichConceptItems` then tolerates the error as `enrichment_failed`. So the guard's own message, "Network call blocked in tests", never reaches the log — only the SDK's "Connection error". Seconds of backoff per concept, across every generation test, push tests past the 20s timeout; vitest cannot cancel the abandoned work, which is T-100's mechanism for failures in whatever runs next.
  - The guard kept the network safe; what broke `loop.md §3` is that the suite still depended on an unmocked model call, and paid for it in time and in a log line that named the wrong thing.
- **acceptance:** No backend test makes a network request to a model provider; the worker suite passes with the network unavailable, in the same time either way.
- **tests:** the worker test mocks `enrichConceptItems` as a pass-through; the full backend suite passes.
- **notes:** (2026-09-15) **Fixed in three files, not one.** A search for tests that run generation found `sprint1.test.ts` and `sprint2.test.ts` making the same unmocked call; all three now mock `enrichConceptItems` as a pass-through over the real module. `itemBlocks.test.ts` already mocks `openai` itself and needed nothing.
  - **Proof:** the full backend suite passed **695/695 in 36 seconds** with **zero** "enrichment call failed" lines in its output — the run before the fix took over fifteen minutes and failed nine tests. The worker file alone went from dozens of connection errors to 16 passes in 1.5 seconds.
  - **Not fully explained:** in the failing run, `POST /reviews > 404s for an unknown item` ran about fifteen minutes. Retry backoff accounts for tests running seconds long, not minutes, and the test database showed no stuck transactions afterwards. Recorded rather than explained away; if a multi-minute test recurs with the enrichment mocked, the cause is elsewhere (T-111).

### T-047 · Error monitoring + health
- **status:** done (Sentry skipped — optional, no DSN)
- **sprint:** 4
- **depends_on:** T-001
- **files:** `backend/src/lib/log.ts`, `backend/src/routes/health.ts`, `backend/src/app.ts`, `backend/src/index.ts`, `backend/src/llm/client.ts`, `backend/src/workers/{generator,tests,lifecycle}.worker.ts`, `packages/shared/src/{schemas,types}.ts`, `frontend/src/app/ErrorBoundary.tsx`, `frontend/src/app/AppShell.tsx`, `frontend/src/app/router.tsx`, tests alongside
...
- **notes:** (2026-09-15) **Split liveness from readiness.** Coolify restarts the container on its health check, so if `/health` starts checking Postgres a database blip becomes a restart loop. Keep `/health` as a cheap liveness probe for Coolify and add a readiness endpoint that checks Postgres and Redis. This is not hypothetical: on 2026-09-13 Coolify reported `running:healthy` through a total database outage (handoff). Also log grader failures (`gradeExplanation`, `gradeCode`, now bounded at 20s by T-171) with the prompt name, so a model outage is visible as one line rather than a scatter of 500s.
  - (2026-09-15) **Done.**
    - **Deviation from the listed tests, on purpose:** they say `/health` returns 503. They run against **`GET /health/ready`** instead, per the note above. `/health` stays a static liveness probe, because it is what Coolify restarts on. A test asserts `/health` stays 200 while both dependencies are down, so the split cannot quietly collapse.
    - **`/health/ready`** runs `select 1` against Postgres and `PING` against Redis in parallel, each bounded at 2s so a hung dependency cannot hang the probe. It answers 200 `{ ok: true, checks: { postgres: 'ok', redis: 'ok' } }` or 503 naming which is `down`. The underlying error goes to the log with the request id, never into the response. Its Redis connection is its own (BullMQ's belong to the queues) and is closed on shutdown. `ReadinessResponseSchema` is in `@learnos/shared`. Tested against the real test Postgres and Redis, plus fakes for Redis down, Postgres unreachable and a check that never answers.
      - `curl -s https://api.coldrecall.info/health/ready`
    - **Structured logs, no dependency** (`lib/log.ts`): one JSON object per line with `time`, `level` and `event`. Credential-looking keys are redacted at any depth, and errors are serialised with their `cause` (drizzle's real error lives there, T-154). Silent under vitest; tests build their own logger with a capturing sink.
    - **Request ids:** `requestLogging` is the first middleware. It reuses a well-formed `X-Request-Id`, otherwise mints one, returns it on every response, and logs `http_request` with method, **path (never the query string, which carries sign-in tokens and OAuth codes)**, status and duration. `/health` is not logged, since Coolify polls it every few seconds.
    - **500s:** the error handler logs `request_failed` with the request id and the serialised error, and the same `requestId` is now in the 500 body, so a learner can report the string that finds the line.
    - **Worker failures:** all three workers log `job_failed` with queue, job id, name, attempts and data, plus `worker_error`. The generation worker had no failure handler at all. `generation_failed` carries the raw model response when there is one.
    - **Model calls:** `complete()` logs `llm_call_failed` with the prompt name, model and duration before rethrowing.
    - **The web app:** `ErrorBoundary` wraps the routed screen inside `AppShell`, so the bar and account menu survive a crashing screen. It says what broke (never a stack) and offers "Try again" and "Back to today", resetting on navigation. **A second, outermost boundary wraps the whole route tree** in `AppRoutes` — beyond the task's "one boundary", and found by the task's own test. With a blanket `{}` API stub, the bar threw on `session?.newConcepts.length` (the `?.` guards only `session`), and the bar sits outside the screen boundary, so the page went completely blank. A test now makes the bar throw and asserts the fallback renders instead. Uses existing classes only.
    - **Skipped:** Sentry (optional, needs a new dependency and a DSN). `componentDidCatch` logs to the console until one exists.
    - **Tests:** backend 712/712 (log 9, health 6, app 4, 17 new); frontend 89 (error boundary 4, outer boundary 1); extension 126; `@learnos/ui` 48; `@learnos/shared` 54; lint 7/7.
    - **Not verified live:** `/health/ready` on the Coolify host needs a deploy. Whether Coolify should also watch readiness, in a way that alerts rather than restarts, is a settings decision for the founder.

### T-175 · A question that asks for code, answered in a one-line box
- **status:** done
- **sprint:** 6
- **severity:** high — the learner cannot give the answer the question asks for, and the grader is comparing against something else again
- **depends_on:** T-088, T-171
- **files:** `packages/shared/src/promptShape.ts`, `packages/shared/src/index.ts`, `backend/src/generator/items.ts`, `backend/src/generator/errors.ts`, `backend/fixtures/items.usestate.json`, `packages/ui/src/QuestionCard.tsx`, `packages/ui/styles/_field.scss`, tests alongside
- **description:** Found by the founder from a screenshot (2026-09-16). The session showed *"Write a component that increments a counter when a button is clicked."* over the generic single-line input, captioned **"A few words is enough"** — while the item's stored answer was a whole JSX snippet. Three different ideas of the answer on one card: the prompt asks for a component, the input fits a phrase, and the grader compares against a snippet.
  - **Why it rendered that way:** the item is a plain `application` item with no answer block, so `QuestionCard` fell through to its default input.
  - **Why nothing caught it:** `prompts/items/domains/code.md`'s decision list already says a capability — "nothing short of writing it is evidence" — must be a `codeEditor`, but that is prose in a prompt file. `validateItems` checked item types, rubric length and block rules, and **never compared the prompt with the answer format**, so a live generation could produce the same card. The one on screen came from `backend/fixtures/items.usestate.json`, written before Sprint 5's formats existed (the same fixtures T-140 recorded as carrying no blocks).
- **acceptance:** An item whose prompt asks the learner to write code either carries a `codeEditor` block or is rejected at generation; no surface shows such a prompt over a one-line input.
- **tests:** shared `wantsWrittenCode` (both directions, including the false positives); generator rejects a code prompt without a block, accepts it with one, and leaves a one-liner alone; `QuestionCard` gives a code prompt a textarea and keeps the short input for a one-liner.
- **notes:** (2026-09-16) All three parts, as the founder asked.
  - **One definition, two readers.** `wantsWrittenCode` lives in `@learnos/shared` because the generator and the renderer have to agree on what "asks for code" means. It is narrow on purpose, in the spirit of `REFERS_TO_SHOWN`: an imperative to *produce* something (`write`, `implement`, `build`, `create`) plus a noun naming a multi-line artefact (`function`, `component`, `hook`, `class`, `module`, `reducer`, `middleware`, `query`, `endpoint`, `handler`, `test`), within 80 characters of each other, and never `write about`. A one-liner — *"Write the setter call"*, *"Write the condition that stops the loop"* — is a good plain item and must not be caught; a false positive costs a regenerated item for nothing.
  - **The generator rule** is a new `code_answer_format` reason, thrown by `validateItems` inside the retry loop, deliberately **not** in `REPAIRABLE_REASONS`: like `dangling_reference`, the learner cannot answer the question as asked, so it is worth one more call. The model's repair is the right move either way — write the `codeEditor` block, or ask a smaller question.
  - **The seed item** now asks what a one-line answer can hold: *"You have `const [count, setCount] = useState(0)`. Write the call that increments it by one."* with `setCount(count + 1)` as the answer and the arrow-function forms accepted. Checked against `wantsWrittenCode` so the new rule does not fire on it.
  - **The card** gives a code-asking prompt a six-row mono textarea instead of the one-line box, with spellcheck and autocapitalise off (they are retrieval cues, T-088). This is a backstop for items already in the database, not the fix — the fix is the generator rule. The e2e card helper already recognises `textarea[aria-label="Your answer"]`, so such a card stays answerable.
