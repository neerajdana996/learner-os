...
- **flows (2026-09-10):**
  ...
  - **Held-out/untaught concepts** can never reach `/due` — asserted at the API only, since it is structurally guaranteed at the query level (see notes). *(`codeEditor` items were excluded the same way until T-171, 2026-09-14. They are served now, so `cardActions.spec.ts` asserts instead that a served `codeEditor` carries neither its expected outputs nor its skeleton.)*
...
- **flows:**
  ...
  - ~~`codeEditor` is absent from the extension's popup (T-089)~~ **Reversed by T-171:** every format, `codeEditor` and `orderLines` included, is served to the side panel and draws its own surface there (`e2e/extension/formats.spec.ts`).
...
- **notes:** (2026-09-10) `backend/src/scripts/seedFormats.ts` is written and confirmed working — 5 items seeded, all passing `ItemPayloadSchema`. **What's still open from T-140's own scope:** confirming a *live* code-topic generation actually emits blocks (today's dev database has zero organically-generated ones). That check is independent of this E2E group and shouldn't block writing these specs against the seeded fixtures.
  ...
  - ~~`codeEditor` is asserted absent from a session, because it is never a review (T-088).~~ **Since T-171 it is a review**, and the session walk now asserts it appears.
  - (2026-09-14) **`e2e/extension/formats.spec.ts` lands — the popup half — and it found a real bug (T-169).** Three tests: the `/due` queue offers every popup-eligible format and neither excluded one; `clozeCode`, `hotspotLine` and `numeric` each draw their own answer surface in the 344px popup (screenshotted and attached, so the contact sheet now covers the small surface too); and a queue holding *only* `orderLines` and `codeEditor` correctly reads "Nothing due right now" rather than serving one of them.
  - **The absence assertion is stronger here than E2E-006's.** `cardActions.spec.ts` polls the dev user, where five formats share one concept and only one can surface per request — it can only ever say "not this draw". The showcase fixture is one card per concept with all fourteen overdue, so a single `/due` call is the complete set, and "absent from a response containing every other concept in the topic" is a different claim.
  - **T-169, found on the first run:** `getDueItems` spends its LIMIT on due *cards* and only then filters *items* by `popupEligible()`, so a due `codeEditor`/`orderLines` card at the head of the queue consumes the one row `Popup.tsx` asks for (`/due?limit=1`) and the popup says "nothing due right now" while answerable cards wait. Not asserted in this file — `findDueCards` orders by `due`, the showcase seeds every card at the same instant, and no API lets a spec give one card a strictly earlier `due`, so a test for it would turn on the tie-break. It belongs in a `due.service` unit test where `now` and the rows are both controlled. `leaveOnly()` answers both ineligible cards away before each popup open to work around it, and says so.
  - (2026-09-15) **T-170 and T-171 changed this group's premises, and the specs follow them.** The extension is a side panel rather than a 344px popup; `orderLines` and `codeEditor` are served there, and `codeEditor` is a web review too. Passing on the new rules: web 38 (1 conditional skip), extension 18, including `a due codeEditor opens in the panel and can be submitted`. The paragraphs about a popup subset above and below are history.
  - **`seedFormatShowcase.ts` now writes an `itemId` per card.** `orderLines` and `codeEditor` are hidden from `/due` by `popupEligible()` and `codeEditor` from every review queue by `reviewEligible()`, so no API can tell a spec their item ids — and without them the only way to clear those two cards out of the popup's path would be to reach into Postgres from the test. The web half reads only `slug`/`prompt` and is unaffected.
