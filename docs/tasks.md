...
**62 of 111 tasks done** (1 in progress, 1 blocked, 47 todo). Sprints 1 and most of 2 are shipped: backend **407 tests**, frontend **49**, extension **31**, all lint-clean.
...
**Next:** `T-082` (the concept map decides each concept's domain) — `loop.md §0`'s first-unblocked rule. `T-085` (the renderer walks blocks) is also unblocked now that `T-080` has landed. `T-092` needs `T-082` as well as `T-095`, so it is not next. Nothing in Sprint 5 jumps the measurement-first order above without a deliberate decision.
