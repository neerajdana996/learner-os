...
### T-048 · Deployment
- **status:** in_progress — `docs/deploy.md` written; the extension release needs the founder
- **sprint:** 4
- **depends_on:** T-047
...
- **notes:** (2026-09-15) **The documentation half is done:** `docs/deploy.md` covers how a push becomes a deploy (the watch-path table, what rebuilds what), checking a deploy with `/health` and `/health/ready` from T-047, every backend and frontend variable by name with where it is set and whether it is a secret (no values), rotating a secret with `set-secrets.py`, rolling back two ways, reading T-047's JSON log events, and releasing the extension. `infra/README.md` links to it, and its folder map no longer lists the deleted legacy `*.tf` files.
  - **The rollback section carries the important caveat:** the backend runs `drizzle-kit push` at every start, so **rolling back code never rolls back the schema**. A data-losing schema change makes the push stop for a confirmation a container cannot give, so that deploy fails safe (the old container keeps serving) but cannot ship by pushing.
  - **Found while writing it — the production extension cannot connect today.** The manifest has no `key`, so the extension has no fixed ID, and in production the API refuses every `chrome-extension://` origin not in `EXTENSION_ORIGINS`, which is unset. Any zip built so far, including the one handed to the founder on 2026-09-14, would show "Could not reach the backend".
  - (2026-09-17) **The ID is pinned in the manifest** (founder's call). `extension/wxt.config.ts` carries a public `key`, so every install is `chrome-extension://gijjgknkbkacdmlbloimmblgicondimf` — the private half was never committed and no build reads it. `extension/src/__tests__/extensionId.test.ts` recomputes the ID from the key the way Chrome does, so a changed key fails the suite instead of silently disconnecting every learner. `infra/coolify/bootstrap.py` now carries the matching `EXTENSION_ORIGINS`, and `docs/deploy.md` has the release steps.
  - (2026-09-17) **`EXTENSION_ORIGINS` is set and live**, by the founder. Verified from outside: `GET /me` with `Origin: chrome-extension://gijjgknkbkacdmlbloimmblgicondimf` comes back 401 **with** `access-control-allow-origin` echoing that origin and `allow-credentials: true`; an unknown `chrome-extension://` origin gets **no** allow-origin header, so it is a real allowlist rather than "any extension"; `https://coldrecall.info` is unaffected. Readiness is 200 with Postgres and Redis both `ok`.
  - **What is left, and it is the founder's:** install `extension/.output/chrome-mv3` (or the zip) with Developer mode, connect it with a token from the web app's **Connect extension** screen, and answer one card — it should reach `review_events` with `surface = extension`. That is the last acceptance line; everything it depends on is now live.
  - Not verified by walking through it: the Coolify rollback step is described from how Coolify's deployments list works, not tested against this instance.
...
- **notes:** (2026-09-13) **Applied**, after the founder reviewed both plans. Each apply ran from a saved plan and was gated on matching the reviewed counts exactly — bootstrap 4/0/0, prod 26/0/0 — so nothing outside what was read could be created.
  ...
  - **Zone verified before any nameserver change**, by asking Route 53 directly (`test-dns-answer`): apex and `www` answer `76.76.21.21`, `api` the legacy CNAME, `deploy` (renamed from `coolify` at the founder's request) the new IP, and MX, SPF, site verification and both DKIM keys match BigRock. Google's 408-character key is stored as two strings of 255 and 153 that rejoin exactly. Switching the nameservers is therefore safe.
  ...
  - **Postgres and Redis are public on 5432 and 6379** (founder decision, 2026-09-13, so they can be used from a laptop), open to `0.0.0.0/0` and protected only by their Coolify-generated passwords. Set in the gitignored `terraform.tfvars` (`database_access_cidrs`); the committed default is closed. **Verified from outside without credentials:** Redis answers `-NOAUTH Authentication required.`, Postgres requires SCRAM (auth code 10), not trust. If either password leaks it is a direct way into production data — narrowing the list to known addresses is a one-variable change.
  - **Coolify stack created** by `infra/coolify/bootstrap.py`: project `coldrecall` (production), Postgres 16 and Redis 7 running healthy, `backend` and `frontend` apps wired to the `coldrecall-git` GitHub App with auto-deploy on push, not yet deployed. Secrets — `DATABASE_URL`, `REDIS_URL`, `OPENAI_API_KEY`, `SMTP_PASS` and both OAuth client secrets — are pasted into Coolify by the founder, never set by tooling. The OpenAI key, SMTP password and OAuth secrets were exposed in a chat on 2026-09-13 and should be rotated.
  - **Dashboard domain is `deploy.coldrecall.info`** (founder, 2026-09-13), not `coolify.`. Set it inside Coolify only **after** the admin account exists: until then the registration page is open to the first visitor.
