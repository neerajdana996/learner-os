...
- **notes:** (2026-09-12) Do not "fix" this by loosening `MAX_RICH_ITEMS` or by adding blocks in post-processing. The cap is a review-time budget (T-083) and the blocks have to be the model's, resolved from line *quotes* by the worker — a generated block is the only kind the answer key can be trusted for.
...
- **notes:** (2026-09-12) Classify the new rule under T-164's severity split when it lands. An invented teach block is decoration on one lesson, not an unusable course — `explanation_not_expanded` is the closest precedent, and it is a preference.
