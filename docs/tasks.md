...
- **notes:** (2026-09-06) Built and verified. Backend 473 → **486 tests**, frontend 49, extension 31; lint green in all three, `pnpm build` green in both clients, sync clean.
  ...
  - ⚠ **One pre-existing test is intermittently red** — `PATCH /me > rejects an invalid patch`, once in about seven full runs, passing 8/8 in isolation and in the twelve full runs around it. Nothing here touches that module. Logged as `T-100` with what was ruled out, rather than guessed at or ignored.
...
### T-100 · One test fails about once in seven full runs, and only in a full run
- **status:** todo
- **sprint:** 5
...
- **description:** Observed while verifying T-083. `PATCH /me > rejects an invalid patch without writing anything` failed once in a clean serial `pnpm test`, and passed in the twelve full runs before and after it. Nothing in T-083 touches the users module or its test.
  - **What is already ruled out.** It is not file parallelism — `vitest.config.ts` sets `fileParallelism: false` precisely because these tests share one Postgres. It is not the test itself: it passes 8/8 when that file is run alone. It is not the rate limiter (T-FIX-007), which keys on email and `seedUser` mints a unique address per call. So it is something process-global that only accumulates across a **full** run, and the cause is genuinely unknown — this task is to find it, not to apply a guessed fix.
...
- **acceptance:** The named test either fails reproducibly and is fixed at its cause, or the cause is identified and written here. Two concurrent `pnpm test` runs both pass.
