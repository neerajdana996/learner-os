...
- **notes-open:** (2026-09-06) Three findings from the same audit, **not** acted on:
  - **The canvas shows no confidence tap on the wrong-answer artboard**, only on the correct one. The build asks on both, and should: collecting confidence only when the learner was right makes overconfidence unmeasurable, and calibration is one of the pilot's numbers (plan.md §3.6). **The artboard is what should change.** Founder's call.
  - **Snooze is a clock icon in the header** in the canvas, beside the ✕, on the asking card only. The build has a "Later" text button in the actions row. An unlabelled clock glyph is ambiguous and the text button is the more accessible of the two, so the divergence stands pending a decision.
  - **No artboard exists for the offline state** added in T-031 — "Saved. You're offline — we'll send this the moment you're back." Worth drawing before the pilot, since it is a state real users will hit on a train.
  - Still open and already documented: the header reads "Due now" rather than the concept name, because `PublicItem` withholds titles for held-out concepts (T-010).
